...
const ScrollContainer: React.FC<ScrollContainerProps> = (props) => {
  ...
  useEffect(() => {
    ...
    if (listLen <= listSize) {
      ...
      listLen &&
        ...
        container.children[0].classList.add('cm-active');
    } else {
  ...
  const polyScroll = () => {
    ...
    box.onscroll = (e) => {
      ...
      if (currentScrollTop - lastScrollTop >= itemHeight) {
        let firstIndex =
          Math.floor(Math.abs(currentScrollTop - baseHeight) / (itemHeight * halfListSize)) *
          halfListSize;
      ...
      if (currentScrollTop - lastScrollTop <= -itemHeight) {
        let firstIndex =
          Math.floor((currentScrollTop - itemHeight) / (itemHeight * halfListSize)) * halfListSize;
  ...
  const bindNewItems = (isScrollDown) => {
    ...
    if (container && container.children && container.children.length) {
      ...
      container &&
        container.children &&
        container.children[newInd] &&
        container.children[newInd].classList.add('cm-active');
      //-end
  ...
  const adjustPaddings = (firstIndex) => {
    ...
    const newCurrentPaddingBottom =
      totalPadding - newCurrentPaddingTop < 0 ? 0 : totalPadding - newCurrentPaddingTop;
...
interface ScrollContainerProps {
  ...
  theme: string;
  selectChange: (item: { name: string; value: string }) => void;
  listLen: number;
  listSize: number;
  itemHeight: number;
  typeMap: { [key: string]: TypeMapItem }; // 使用索引签名替代Record
  lang: string;
  domId: string;
}

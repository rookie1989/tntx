...
interface FormulaEditProps {
  ...
  editorEvent?: (event: {
    ...
    fullScreen: () => void;
    exitFullScreen: () => void;
    EnCodeToCn: (code: string) => string;
    CnCodeToEn: (code: string) => string;
  }) => void;
  placeholder?: string;
  cnCodeToEnExtraLogic?: (item: FieldItem) => string;
  enCodeToCnExtraLogic?: (code: string) => string;
  lang?: string;
  searchCb?: (params: {
    field: string;
    type: string;
    ...
    fieldList: FieldItem[];
    methodList: MethodItem[];
    searchList: (FieldItem | MethodItem)[];
    CnCodeToEn: (code: string) => string;
    EnCodeToCn: (code: string) => string;
  }) => (FieldItem | MethodItem)[];
  onChange?: (
    enCode: string,
    data: { cnCode: string; enCode: string; errorMsg: string | null },
  ) => void;
}
...
// 修改组件定义
const FormulaEdit = forwardRef<HTMLDivElement, FormulaEditProps>((props, ref) => {
  const {
    children,
    value = '',
    readOnly = false,
  ...
  const [dropList, setDropList] = useState<(FieldItem | MethodItem)[]>([]);

  const codeMirrorEditor = useRef<CodeMirror.Editor>();
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const fieldRegExpRef = useRef<RegExp>(new RegExp(''));
  const funRegExpRef = useRef<RegExp>(new RegExp(''));
  const funRegExpGRef = useRef<RegExp>(new RegExp('', 'g'));
  const normalExpGRef = useRef<RegExp>(new RegExp('', 'g'));
  const domId = useRef<string>(getId());
  const modeType = useRef<string>(getId('defineScript'));
  const modeField = useRef<{
    codemirrorFieldList: string[];
    codemirrorMethodList: string[];
    codemirrorNormalList: string[];
    codemirrorKeywordList: string[];
  }>();
  const eventRef = useRef<(cm: CodeMirror.Editor) => void>();

  const tntCodeMirrorRef = useRef<HTMLDivElement>(null);
  ...
  // 全屏
  const fullScreen = () => {
    codeMirrorEditor.current.setOption(
      'fullScreen',
      !codeMirrorEditor.current.getOption('fullScreen'),
    );
  ...
  const setLocalStorage = () => {
    ...
    const fArr = (fieldList || []).map(
      (item) => `@${item.name.replace(/\[/g, '\\[').replace(/\]/g, '\\]')}`,
    );
  ...
  useEffect(() => {
    ...
    editorEvent &&
      editorEvent({
        codeEditor: codeMirrorEditor.current,
        fullScreen,
        exitFullScreen,
        EnCodeToCn,
        CnCodeToEn,
  ...
  useEffect(() => {
    if (codeMirrorEditor.current) {
      codeMirrorEditor.current.addKeyMap({
        Up: (cm: CodeMirror.Editor) => {
          enterFuc('up', cm);
        },
        Down: (cm: CodeMirror.Editor) => {
          enterFuc('down', cm);
        },
  ...
  const cursorActivity = (cm) => {
    ...
    if (selectStyle) {
      ...
      const { x, width } = tntCodeMirrorRef.current
        ? tntCodeMirrorRef.current.getBoundingClientRect()
        : {};
  ...
  const handleClick = (item, type) => {
    ...
    codeMirrorEditor.current.setCursor(
      getCursor.line,
      lastIndex + 1 + content.length + offsetIndex,
    );
    codeMirrorEditor.current.focus();
    setCurState({
      ...curState,
